...
- **本地图库** 自动汇总生成历史，并可从照片/Files 导入、在桌面端扫描文件夹;按 Prompt、模型、日期、分类、收藏和集合检索,
  支持多选保存、分享、分类、集合与移出;生成图片只保留一份原图，外部导入文件单独托管

- **在线画廊** 支持 Danbooru、Safebooru、Gelbooru、AI TAG 与法典图鉴来源切换,
  提供搜索、排行、日期/分级筛选、黑名单、收藏、详情及提示词/参考图回填

- **可复用资源** 固定词库支持正负向、前后置、权重与启停;新增精准参考库,保存参考类型、强度、保真度和收藏状态
...
| 计划 | 说明 | 阶段 |
|---|---|---|
| **多 Token 管理** | 多账号保存、主副账号切换与并发生成 | 已完成 |
